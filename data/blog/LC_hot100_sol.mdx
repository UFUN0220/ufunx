...
利用**回溯算法（\*\***DFS\***\*）构建一棵多叉树，树的每一层对应输入的一个数字，通过遍历该数字映射的所有字母分支，并在\*\***递归\***\*返回时撤销选择**（Pop Back），从而穷举所有可能的字符组合。
...
结合 **快慢指针** 和 **链表\*\***反转** 技巧，首先利用快慢指针定位链表的**中点**，随后将**后半部分\*\*链表进行原地反转，最后通过双指针同步比较前半段与反转后的后半段是否一致，从而在 $O(N)$ 时间且 $O(1)$ 空间内完成回文校验。
...
利用**动态规划**（完全背包问题的变体），定义 `dp[j]` 为凑成金额 `j` 所需的**最少\*\***硬币\***\*数量**。通过外层遍历硬币类型、内层**正向遍历**金额（从小到大，意味着允许**重复使用**同一面额），在每次迭代中尝试加入当前硬币，并取“当前已知最优解”与“加入新硬币后的解”的**最小值**来更新状态。
...
利用 **滑动窗口** (Sliding Window) 结合 **双指针** 和 **哈希表** 技巧。

核心逻辑是维护一个**固定长度**为 `p.size()` 的窗口在字符串 `s` 上滑动：

1. **右移扩大** (`right`)：不断加入新字符并更新窗口内的计数器 `window` 和匹配状态 `valid`。
2. **左移收缩** (`left`)：一旦窗口大小达到 `p` 的长度，便尝试收缩。在收缩前，检查当前窗口是否完全匹配（即 `valid` 数量达标），若是则记录起始索引。
3. **状态维护**：通过 `valid` 变量记录“已满足频次要求的字符种类数”，避免了每次比较两个哈希表的 $O(26)$ 开销，将复杂度优化至 $O(N)$。
...
利用 **异或\*\***运算** (XOR) 和 **位操作**。首先计算 $x \oplus y$，该操作会将两个数字二进制表示中**不同**的位标记为 `1`，相同的位标记为 `0`。随后，通过循环不断检查运算结果的**最低有效位** (`s & 1`) 是否为 1 并累加计数，同时将数字**右移\*\* (`s >>= 1`)，从而统计出所有不同位的数量。
...
利用 **二叉搜索树** **(\*\***BST\***\*)** 的有序性，采用 **反向\*\***中序遍历**（$Right \to Root \to Left$）。由于 BST 的右子节点大于根节点，这种遍历方式保证了节点严格按照 **从大到小\*\* 的顺序被访问。
